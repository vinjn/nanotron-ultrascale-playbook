...
1.  [Fundamentals of NVIDIA GPU architecture](https://www.aleksagordic.com/blog/matmul#cpt1): global memory, shared memory, L1/L2 cache, impact of power throttling on SOL, etc.
2.  [GPU assembly languages](https://www.aleksagordic.com/blog/matmul#cpt2): SASS and PTX
3.  [Designing near-SOTA synchronous matmul kernel](https://www.aleksagordic.com/blog/matmul#cpt3): the warp-tiling method
4.  [Designing SOTA asynchronous matmul kernels on Hopper](https://www.aleksagordic.com/blog/matmul#cpt4): leveraging tensor cores, TMA, overlapping computation with loads/stores, Hilbert curves, etc.
...
[Hopper](https://developer.nvidia.com/blog/nvidia-hopper-architecture-in-depth/) [\[1\]](https://www.aleksagordic.com/blog/matmul#ref-1) and [Ampere](https://developer.nvidia.com/blog/nvidia-ampere-architecture-in-depth/) [\[2\]](https://www.aleksagordic.com/blog/matmul#ref-2) white papers are a great source of information.
...
![Figure 1: Model of the NVIDIA Hopper H100 GPU](images/h100_model_8ee6a76e3b9c.png)
...
![Figure 2: Memory hierarchy of the H100 (SXM5) GPU](images/mem_hierarchy_a9f61b858791.png)
...
![Figure 3: H100 SXM5 BF16 speed-of-light derivation](images/h100_sol_9cf3af00308c.png)
...
![Figure 4: Power throttling reduces clock frequency and lowers the effective “speed of light”](images/clk_821585f3893b.png)

Figure 4: Power throttling reduces clock frequency and lowers the effective “speed of light”

📝Further reading:

Horace He went into this phenomenon in more depth in his [his blog post](https://www.thonking.ai/p/strangely-matrix-multiplications) [\[3\]](https://www.aleksagordic.com/blog/matmul#ref-3).
...
![Figure 5: CUDA Programming Model: threads, warps, blocks, clusters, grids](images/cuda_model_0d49d6640ea9.png)
...
![Figure 6: CUDA's built-in variables: how threads know where they are](images/cuda_model2_a3c6cd90f91e.png)
...
![Figure 7: Inside a DRAM cell: transistor + capacitor, wordline + bitline](images/gmem_dram_cell_0c5a44371f51.png)
...
![Figure 8: GMEM model](images/gmem_dbb04e9bcda6.png)

Figure 8: GMEM model

📝Further reading on HBM:

If you want to dive deeper into HBM, I found the paper ["Demystifying the Characteristics of High Bandwidth Memory for Real-Time Systems"](https://upcommons.upc.edu/server/api/core/bitstreams/b843de39-f32f-4069-8843-48f74c030213/content) [\[21\]](https://www.aleksagordic.com/blog/matmul#ref-21) fairly informative.

Thus we conclude: access patterns matter because of the physics of DRAM cells. Here is an example:

![Figure 9: Effect of access pattern in GMEM](images/gmem_example_9ffead5b05f6.png)

Figure 9: Effect of access pattern in GMEM

Stephen Jones's talk ["How CUDA Programming Works"](https://www.nvidia.com/en-us/on-demand/session/gtcfall22-a41101/) [\[4\]](https://www.aleksagordic.com/blog/matmul#ref-4) is a good watch.
...
![Figure 10: SMEM model](images/smem_pt1_349393f401de.png)
...
![Figure 11: SMEM: good vs. bad access patterns](images/smem_pt2_651002e5fc35.png)
...
![Figure 12: SMEM: multicasting (served in a single cycle)](images/smem_pt3_2c95f4f40ae5.png)

Figure 12: SMEM: multicasting (served in a single cycle)

Now, for the final piece of the hardware puzzle: the L1 cache.

This is an excellent [blog post](https://feldmann.nyc/blog/smem-microbenchmarks) [\[5\]](https://www.aleksagordic.com/blog/matmul#ref-5) by Alex on SMEM microbenchmarking.
...
![Figure 13: L1 cache model](images/l1_8e982c046b55.png)

Figure 13: L1 cache model

Let's go one level deeper and look at the tag stage and data stage in detail:

![Figure 14: Breakdown of k-way set-associative cache organization](images/kway_c6c094a8e80e.png)
...
![Figure 15: CUDA compilation flow: from CUDA C++ → PTX → SASS](images/ptx_sass_hierarchy_aa3bd47ee7e9.png)
...
For a deeper dive into SASS, I recommend Aroun's ["Introduction to SASS & GPU Microarchitecture"](https://www.youtube.com/watch?v=we3i5VuoPWk) [\[7\]](https://www.aleksagordic.com/blog/matmul#ref-7) video.
...
A great example of the performance hidden in these low-level details comes from the now-famous Citadel paper, ["Dissecting the NVIDIA Volta GPU Architecture via Microbenchmarking"](https://arxiv.org/abs/1804.06826) [\[8\]](https://www.aleksagordic.com/blog/matmul#ref-8). The authors tweaked SASS to avoid memory bank conflicts and boosted performance from 132 GFLOP/s to 152 GFLOP/s — a 15.4% improvement.
...
![Figure 16: Naive CPU matmul example](images/cpu_kernel_example_6bf0d586d9a3.png)
...
With that insight, let's look at the simplest possible GPU kernel. We'll use a slightly more general form: `C = alpha * A @ B + beta * C`. This is the classic GEMM (General Matrix Multiply). Setting `alpha = 1.0` and `beta = 1.0` recovers the simpler `C = A @ B`.
...
![Figure 17: Tile quantization](images/tile_quantization_20e073fba249.png)
...
I originally adapted this code from [Simon's blog](https://siboehm.com/articles/22/CUDA-MMM) [\[9\]](https://www.aleksagordic.com/blog/matmul#ref-9) and focused on doing an in-depth PTX/SASS analysis on it (coming up soon), so I didn't want to repeat the hard work as slight code changes would lead to different PTX/SASS.
...
![Figure 18: Thread organization in naive matmul kernel](images/simple_kernel_logical_9c5036fe599a.png)

Figure 18: Thread organization in naive matmul kernel

And the matmul logic itself looks like this:

![Figure 19: Naive matmul kernel](images/simple_kernel_63cad3554c4a.png)
...
![Figure 20: Nsight Compute: Occupancy, Waves info](images/occupancy_waves_08298cc87d56.png)

Figure 20: Nsight Compute: Occupancy, Waves info

Here is an [excellent tutorial](https://www.youtube.com/watch?v=F_BazucyCMw&ab_channel=GPUMODE) [\[11\]](https://www.aleksagordic.com/blog/matmul#ref-11) on using Nsight Compute profiler.
...
![Figure 21: Wave quantization](images/wave_quant_90e8eef84c71.png)
...
![Figure 22: PTX code corresponding to naive matmul CUDA kernel](images/ptx_6ddbe5f8c8fc.png)
...
![Figure 23: SASS code corresponding to naive matmul CUDA kernel](images/sass_8a52ed55de84.png)
...
I highly recommend Simon's excellent [blog post](https://siboehm.com/articles/22/CUDA-MMM). It served as my original inspiration to get deeper into kernels. In this chapter, I'll use his [kernel 10](https://github.com/siboehm/SGEMM_CUDA/blob/master/src/kernels/10_kernel_warptiling.cuh) [\[12\]](https://www.aleksagordic.com/blog/matmul#ref-12) code as the reference. While the code itself appears to be CUTLASS-derived (see [this](https://developer.nvidia.com/blog/cutlass-linear-algebra-cuda/) [\[13\]](https://www.aleksagordic.com/blog/matmul#ref-13), and [this](https://github.com/NVIDIA/cutlass/blob/b0e09d7cd371eded41f7c1e057caf1593c27ba55/media/docs/efficient_gemm.md) [\[14\]](https://www.aleksagordic.com/blog/matmul#ref-14) for example), I first analyzed Simon's version — so that's the one I'll follow here.
...
![Figure 24: New logical organization of row2 and col2 variables](images/simple_kernel_uncoalesced_logical_a1578a893051.png)

Figure 24: New logical organization of row2 and col2 variables

And here's what the modified kernel does now:

![Figure 25: Naive kernel with uncoalesced GMEM access](images/simple_kernel_uncoalesced_a98a3759e630.png)
...
![Figure 26: Roofline model](images/roofline_5d1c148e0a69.png)
...
![Figure 27: Matmul as a sum of partial outer products](images/outer_d86b0b6286a8.png)
...
![Figure 28: Dot product is equivalent to a sum of partial dot products](images/block_tile_equivalence_ba9329f3ef36.png)
...
![Figure 29: Arithmetic intensity improves when each thread computes multiple outputs and when tiles approach a square shape](images/arithmetic_intensity_438eb3c269e8.png)
...
![Figure 30: High-level structure of the warp-tiling algorithm, also referred to as block tiling.](images/warp_tiling_pt1_5306a1ba1e79.png)
...
![Figure 31: Loading a chunk of B (GMEM) into Bs (SMEM)](images/warp_tiling_pt2_smem_load_b_636ace818843.png)
...
![Figure 32: Loading a chunk of A (GMEM) into As (SMEM)](images/warp_tiling_pt2_smem_load_c438d970cf53.png)
...
![Figure 33: Performing matmul between As and Bs as a series of thread-level outer products (warp-tiling + thread-tiling).](images/warp_tiling_pt3_compute_130a37f65f09.png)
...
I highly recommend Pranjal's excellent [blog post](https://cudaforfun.substack.com/p/outperforming-cublas-on-h100-a-worklog) [\[15\]](https://www.aleksagordic.com/blog/matmul#ref-15), which reads more like a worklog. In this chapter, I'll be following kernels from his worklog. As with Simon's work, much of the code appears to be CUTLASS-inspired (see, for example, these posts: CUTLASS [ping pong kernel](https://pytorch.org/blog/cutlass-ping-pong-gemm-kernel/) [\[16\]](https://www.aleksagordic.com/blog/matmul#ref-16) and [efficient GEMM](https://github.com/NVIDIA/cutlass/blob/b0e09d7cd371eded41f7c1e057caf1593c27ba55/media/docs/efficient_gemm.md)).
...
I'll be following [kernel 2](https://github.com/pranjalssh/fast.cu/blob/main/examples/matmul/matmul_2.cuh) [\[17\]](https://www.aleksagordic.com/blog/matmul#ref-17) as the reference here (see also my [PR](https://github.com/pranjalssh/fast.cu/pull/8/files)). Note that the notation has changed slightly from Simon's: `As` → `sA` and `Bs` → `sB`.
...
As a first step toward Hopper SOTA, let's modify the warp-tiling baseline. We'll keep the same high-level program structure, except:
...
![Figure 34: We keep the same high-level structure of the warp-tiling algorithm (block-tiling).](images/high_level_block_structure_5640c6299e07.png)
...
![Figure 35: Swizzling example](images/swizzle_example_942f2ad1e85c.png)
...
![Figure 36: No bank conflicts when loading rows or columns](images/swizzle_property_e56ada57f3f7.png)
...
![Figure 37: Swizzle function intuition](images/swizzle_function_intuition_faf268e7e08f.png)
...
![Figure 38: Connecting the swizzle function to the matrix swizzle example](images/swizzle_apply_16982d9366ce.png)
...
*   `volatile` tells the compiler the asm block \*must not\* be deleted or hoisted, even if it looks redundant (see [docs](https://docs.nvidia.com/cuda/inline-ptx-assembly/#incorrect-optimization)) [\[20\]](https://www.aleksagordic.com/blog/matmul#ref-20).
...
`wgmma.commit_group 0` \- means: don't go any further until every group prior to this point has finished. Since we only launched one group here, it's just saying "hold on until those four MMAs are done and the results are actually sitting in the accumulator registers".
...
![Figure 39: Why doing four 64x16 @ 16x64 wgmma calls is equivalent to doing a 64x64 @ 64x64 matmul](images/4_wgmma_calls_c08b9966ba72.png)
...
![Figure 40: We are wasting TMA and TC cycles - we can do better](images/better_pipeline_12899786c08c.png)
...
![Figure 41: More efficient TC/TMA pipeline: producer warp-group streams tiles into a circular buffer; consumer warp-group drains tiles into tensor cores.](images/kernel4_pipeline_12a260d39d90.png)
...
![Figure 42: Two consumer warp groups let us grow the tile from 128x128 -> 128x256 without register spills](images/2wgs_b5e9ea8f9e14.png)
...
![Figure 43: Persistent kernels: overlap the output store with incoming loads by launching one long-lived block per SM that processes many tiles.](images/overlap_stores_688f63294c2b.png)
...
![Figure 44: Naïve schedule](images/naive_schedule_94473507935a.png)

Figure 44: Naïve schedule

Can we do better? Yes—by making the schedule cache-aware:

![Figure 45: Block-wise cache-aware schedule](images/blockwise_schedule_9f30c858af5d.png)

Figure 45: Block-wise cache-aware schedule

But can we do even better? Surprisingly, yes—by using a space-filling curve:

![Figure 46: Hilbert-curve schedule](images/hilbert_schedule_22c81af3bb52.png)
...
![Figure 47: Using thread block clusters to reduce the number of L2/GMEM loads.](images/clusters_21a30a9b25df.png)
