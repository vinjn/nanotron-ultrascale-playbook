...
![http://blog.ezyang.com/img/pytorch-internals/slide-01.png](images/slide-01_ae18af3839bc.png)

Hi everyone! Today I want to talk about the internals of [PyTorch](https://pytorch.org/).

![http://blog.ezyang.com/img/pytorch-internals/slide-02.png](images/slide-02_fc33fc299651.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-03.png](images/slide-03_d1ca8ef5e1a9.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-04.png](images/slide-04_d3fdab3b29aa.png)

![http://blog.ezyang.com/img/pytorch-internals/slide-05.png](images/slide-05_7431db5ccfd9.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-06.png](images/slide-06_ae1a0619c1a7.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-07.png](images/slide-07_03e6b22064cf.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-08.png](images/slide-08_00edf65e2465.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-09.png](images/slide-09_5239e612d466.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-10.png](images/slide-10_1605ebc3de35.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-11.png](images/slide-11_2717b53814c8.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-12.png](images/slide-12_a1f3736c9a51.png)

The first dispatch is based on the device type and layout of a tensor: e.g., whether or not it is a CPU tensor or a CUDA tensor (and also, e.g., whether or not it is a strided tensor or a sparse one). This is a dynamic dispatch: it's a virtual function call (exactly where that virtual function call occurs will be the subject of the second half of this talk). It should make sense that you need to do a dispatch here: the implementation of CPU matrix multiply is quite different from a CUDA implementation. It is a _dynamic_ dispatch because these kernels may live in separate libraries (e.g., libcaffe2.so versus libcaffe2\_gpu.so), and so you have no choice: if you want to get into a library that you don't have a direct dependency on, you have to dynamic dispatch your way there.
...
![http://blog.ezyang.com/img/pytorch-internals/slide-13.png](images/slide-13_c23754490292.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-14.png](images/slide-14_e688591f7347.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-19.png](images/slide-19_cb9f4e3a1efe.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-20.png](images/slide-20_de8e91c3bb59.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-21.png](images/slide-21_e7de0736d358.png)

![http://blog.ezyang.com/img/pytorch-internals/slide-22.png](images/slide-22_ba83a9756ab8.png)

![http://blog.ezyang.com/img/pytorch-internals/slide-23.png](images/slide-23_7761fe4f1cfa.png)

![http://blog.ezyang.com/img/pytorch-internals/slide-24.png](images/slide-24_6e90de5a4181.png)

![http://blog.ezyang.com/img/pytorch-internals/slide-25.png](images/slide-25_4b114a31407f.png)

![http://blog.ezyang.com/img/pytorch-internals/slide-26.png](images/slide-26_7cdffbeb5325.png)

![http://blog.ezyang.com/img/pytorch-internals/slide-27.png](images/slide-27_1e82cae7c102.png)

***

![http://blog.ezyang.com/img/pytorch-internals/slide-28.png](images/slide-28_91e64b62b79a.png)

Enough about concepts, let's look at some code.

![http://blog.ezyang.com/img/pytorch-internals/slide-29.png](images/slide-29_ce721cc0a482.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-30.png](images/slide-30_a13d71d25887.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-31.png](images/slide-31_53227663a454.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-33.png](images/slide-33_e7ee0b14ee0c.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-34.png](images/slide-34_ec2314c0971b.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-35.png](images/slide-35_483cc4d954b3.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-36.png](images/slide-36_101eda9db656.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-37.png](images/slide-37_6b84c2af3d7c.png)

To take advantage of all of the code generation which PyTorch brings, you need to write a _schema_ for your operator. The schema gives a mypy-esque type of your function, and also controls whether or not we generate bindings for methods or functions on Tensor. You also tell the schema what implementations of your operator should be called for given device-layout combinations. Check out the [README in native](https://github.com/pytorch/pytorch/blob/master/aten/src/ATen/native/README.md) is for more information about this format.

![http://blog.ezyang.com/img/pytorch-internals/slide-38.png](images/slide-38_3e559ecd7837.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-39.png](images/slide-39_765fd5c73885.png)

Error checking can be done by way of either a low level or a high level API. The low level API is just a macro, TORCH\_CHECK, which takes a boolean, and then any number of arguments to make up the error string to render if the boolean is not true. One nice thing about this macro is that you can intermix strings with non-string data; everything is formatted using their implementation of operator<<, and most important data types in PyTorch have operator<< implementations.
...
![http://blog.ezyang.com/img/pytorch-internals/slide-40.png](images/slide-40_e5366442a696.png)

One important thing to be aware about when writing operators in PyTorch, is that you are often signing up to write _three_ operators: abs\_out, which operates on a preallocated output (this implements the out= keyword argument), abs\_, which operates inplace, and abs, which is the plain old functional version of an operator.

Most of the time, abs\_out is the real workhorse, and abs and abs\_ are just thin wrappers around abs\_out; but sometimes writing specialized implementations for each case are warranted.

![http://blog.ezyang.com/img/pytorch-internals/slide-41.png](images/slide-41_c60431ef679c.png)

To do dtype dispatch, you should use the AT\_DISPATCH\_ALL\_TYPES macro. This takes in the dtype of the tensor you want to dispatch over, and a lambda which will be specialized for each dtype that is dispatchable from the macro. Usually, this lambda just calls a templated helper function.

This macro doesn't just "do dispatch", it also decides what dtypes your kernel will support. As such, there are actually quite a few versions of this macro, which let you pick different subsets of dtypes to generate specializations for. Most of the time, you'll just want AT\_DISPATCH\_ALL\_TYPES, but keep an eye out for situations when you might want to dispatch to some more types. There's guidance in [Dispatch.h](https://github.com/pytorch/pytorch/blob/21ef4cc615a7d9d772ade52a5023900718b09e92/aten/src/ATen/Dispatch.h#L62) for how to select the correct one for your use-case.

![http://blog.ezyang.com/img/pytorch-internals/slide-43.png](images/slide-43_aaa16de5d2ca.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-42.png](images/slide-42_1d58cde12233.png)
...
3.  For true speed on CPU, you may need to write your kernel using vectorized CPU instructions. We've got helpers for that too! The Vec256 class represents a vector of scalars and provides a number of methods which perform vectorized operations on them all at once. Helpers like binary\_kernel\_vec then let you easily run vectorized operations, and then finish everything that doesn't round nicely into vector instructions using plain old instructions. The infrastructure here also manages compiling your kernel multiple times under different instruction sets, and then testing at runtime what instructions your CPU supports, and using the best kernel in those situations.

![http://blog.ezyang.com/img/pytorch-internals/slide-44.png](images/slide-44_c1a8fd9210b1.png)
...
2.  It's manually refcounted (with manual calls to THTensor\_free to decrease refcounts when you're done using tensors), and
3.  It lives in generic/ directory, which means that we are actually going to compile the file multiple times, but with different #define scalar\_t.
...
![http://blog.ezyang.com/img/pytorch-internals/slide-45.png](images/slide-45_579ce907c6ea.png)

![http://blog.ezyang.com/img/pytorch-internals/slide-46.png](images/slide-46_da962ed06b7a.png)
...
![http://blog.ezyang.com/img/pytorch-internals/slide-47.png](images/slide-47_a5d7e49c5edb.png)
...
Even if you don't want to get started with coding right away, there are many other useful activities like improving documentation (I _love_ merging documentation PRs, they are so great), helping us reproduce bug reports from other users, and also just helping us discuss RFCs on the issue tracker. PyTorch would not be where it is today without our open source contributors; we hope you can join us too!
