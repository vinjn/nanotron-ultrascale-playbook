...
中文翻译: [xinyu66](https://huggingface.co/xinyu66) 校对: [zhongdongy](https://huggingface.co/zhongdongy)

本文也提供英文版本 [English](https://huggingface.co/blog/moe)。
...
![Switch Layer](images/00_switch_transformer_23a37972cdb6.png)
...
![MoE layer in LSTM](images/01_moe_layer_712a47bd9753.png)
...
y\=∑i\=1nG(x)iEi(x)y\=i\=1∑n​G(x)i​Ei​(x)
...
Gσ(x)\=Softmax(x⋅Wg)Gσ​(x)\=Softmax(x⋅Wg​)

Shazeer 等人的工作还探索了其他的门控机制，其中包括带噪声的 TopK 门控 (Noisy Top-K Gating)。这种门控方法引入了一些可调整的噪声，然后保留前 k 个值。具体来说:

1.  添加一些噪声

H(x)i\=(x⋅Wg)i+StandardNormal()⋅Softplus((x⋅Wnoise)i)H(x)i​\=(x⋅Wg​)i​+StandardNormal()⋅Softplus((x⋅Wnoise​)i​)

2.  选择保留前 K 个值

KeepTopK(v,k)i\={viif vi is in the top k elements of v,−∞otherwise.KeepTopK(v,k)i​\={vi​−∞​if vi​ is in the top k elements of v,otherwise.​
...
![MoE Transformer Encoder](images/02_moe_block_4fe88debb8fa.png)
...
![Switch Transformer Layer](images/03_switch_layer_22d1fdfafd12.png)
...
Expert Capacity\=(tokens per batchnumber of experts)×capacity factorExpert Capacity\=(number of expertstokens per batch​)×capacity factor
...
![Table shows that selective precision does not degrade quality.](images/04_switch_table_eb5400c3c504.png)
...
![Experts specialize in some token groups](images/05_experts_learning_a0dd6ebb3434.png)
...
![Fine-tuning learning curves](images/06_superglue_curves_41a7d21c46b1.png)
...
![Only updating the non MoE layers works well in fine-tuning](images/07_superglue_bars_38a8ed049ce7.png)
...
![Table comparing fine-tuning batch size and learning rate between dense and sparse models.](images/08_superglue_dense_vs_sparse_165373c3ad35.png)
...
![MoEs benefit even more from instruct tuning than dense models](images/09_fine_tune_evals_9236666d52f3.png)
...
![Image illustrating model, expert, and data prallelism](images/10_parallelism_deae201d775c.png)
...
![Matrix multiplication optimized for block-sparse operations.](images/11_expert_matmuls_911d1f32a126.png)
